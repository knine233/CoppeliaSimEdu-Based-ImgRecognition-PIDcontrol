...
class Reporter:
    ...
    def inflate_obstacles(self):
        ...
        if np.max(self._obstacle_mask) > 255 or np.min(self._obstacle_mask) < 0:
            # 假设您希望将值缩放到 0-255 范围内
            self._obstacle_mask = (self._obstacle_mask - np.min(self._obstacle_mask)) * (
                        255 / (np.max(self._obstacle_mask) - np.min(self._obstacle_mask))).astype(np.uint8)
        else:
            # 如果值已经在 0-255 范围内，则直接转换类型
            self._obstacle_mask = self._obstacle_mask.astype(np.uint8)
